...
void LineerYoklamaKareOrt(struct Kayit lineer[500])
{
	...
	for (int i = 0; i < 500; i++)
	{
		bglAdres = kareOrtasi(lineer[i].numara);
		evAdres = bglAdres;
	basla:
...
int lineerYoklamaKareOrtAra(struct Kayit lineer[500])
{
	...
	for (int i = 0; i < 500; i++)
	{
		bglAdres = kareOrtasi(lineer[i].numara); adimSayici++;
		evAdres = bglAdres; adimSayici++;
	basla:adimSayici++;
...
int main()
{
	...
	cout<< setw(5) << lineerYoklamaBolenKalanAra(kisiler)<<" Adimda Lineer Yoklama - Bolen Kalan ile Tum Kisiler Tarandi!"<<endl;//Lineer Yoklama ve Bolen Kalan Kayýt edilen dosyadan tüm kiþiler aranýyor ve ekrana numara isim yazdýrýlýyor.

	LineerYoklamaKatlama(kisiler);

	cout <<setw(5)<< lineerYoklamaKatlamaAra(kisiler)<< " Adimda Lineer Yoklama - Katlama ile Tum Kisiler Tarandi!" << endl;
	
	LineerYoklamaKareOrt(kisiler);
	cout << setw(5) << lineerYoklamaKareOrtAra(kisiler) << " Adimda Lineer Yoklama - Kare Ortasi ile Tum Kisiler Tarandi!" << endl;
	//cout << kareOrtasi(190202101);
