...
int ekran=initwindow(1240,680);
    //Garafik Ekran Yükleme Hatalary
    errcode=graphresult();
    if(errcode!=0)
    {
...
//int baslangicsehri,bitissehri,yolindeksi;  buraya matrisigönder ordan  iþlem yapcan
...
for(i=0;i<dugumsayisi;i++)//HARFLERÝ YAZDIR
{
char s1[20]="D",s2[20];
sprintf(s2,"%d",i);
strcat(s1,s2);
...
setcolor(RED);
setlinestyle(0,10,3);
for(i=0;i<sayac;i++)//KRUSKAL SONUÇ ÇÝZGÝSÝ
{
yol=kruskalsehir[i];
sehirnereden=Matris[yol][nereden];
sehirnereye=Matris[yol][nereye];
...
//BU DONGUDE YOL UZUNLUKLARINI SIRALAYACAÐIZ AYNI ZAMANDA YOL ÝNDEXLERÝDE YER DEÐÝÞTÝRECEK
...
for(int k=0;k<j;k++)
{
     if (matris[k][uzaklik]>matris[j][uzaklik])
      {
        //YOL UZUNLUKLARI YER DEÐÝÞTÝRÝYOR
        g1=matris[k][nereden];
        g2=matris[k][nereye];
        g3=matris[k][uzaklik];
...
void CihanCihatMehmetKuruskal(int dugumsayisi,int Matris[][4],int Dugum[][2],int YedekMatris[][4])//DÝKKAT ALAN KODUNU ve Bölge kodunu dugumsayisi kadar DOLDUR
{
int yoltoplamuzunluk=0,alansirasi=0,alankodu[dugumsayisi],bolgekodu[dugumsayisi],kruskalsehir[dugumsayisi],sayac=0;/*BÖLGE KODU HER DÜÐÜMÜN BAÞLANGIÇ DEÐERÝNÝ GÖSTERÝR
EÐER DÜÐÜM GEZÝLÝRSE BAÞLANGIÇ DEÐERÝNE ALAN KODU ATANACAK bolgekodu nun her bir deðeri aslýnda bir köyün alan kodunu
göstermektedir. bolgekodu[4] yani 4 tane dugumun gezilip gezilmediði durumununu belirtir 0 gezilmemiþ anlamýndadýr alankodu
ayný zaman diliminde gezilip gezilmediðini belirtir.*/
 //alankodu[dugumsayisi]={100,200,300,400},bolgekodu[4]={0,0,0,0};/*ÖRNEKTÝR
int i,f=0;
 for(i=1;i<=dugumsayisi;i++)
 {
...
for(s=0;s<dugumsayisi*dugumsayisi;s++)// TÜM YOL SORGULARindex YAPILIYOR
{
    ...
    //index=yolindex[s];
    if(Matris[s][uzaklik]>0)//YOL UZUNLUÐU 0 DEÐÝLSE
    {
...
if(bolgekodu[baslangic]==0&&bolgekodu[bitis]==0) //gidilen þehirlere ilk kez mi gidilmiþ ilk kez se baþlangýç  0 deðilse  alan kodu atayacaz
{
...
alansirasi++;// bir sonraki sorgulamada eðer 2 þehirde gezilmemiþse farklý bölge kodu atanacak
yoltoplamuzunluk=yoltoplamuzunluk+Matris[s][uzaklik];
kruskalsehir[sayac]=Matris[s][yolindex];
sayac=sayac+1;
}
else if(bolgekodu[baslangic]!=0&&bolgekodu[bitis]==0)//1. þehir gezilmiþ 2. þehir gezilmemiþse
{
...
outtextxy(600,300+f,str); f+=30;
bolgekodu[bitis]=bolgekodu[baslangic];//AYNI YOL DÜZERGAHINA BAÐLANMIÞ OLDULAR;
yoltoplamuzunluk=yoltoplamuzunluk+Matris[s][uzaklik];
...
else if(bolgekodu[baslangic]==0&&bolgekodu[bitis]!=0)//1. þehir gezilmemiþ 2. þehir gezilmiþse
{
...
bolgekodu[baslangic]=bolgekodu[bitis];//AYNI YOL DÜZERGAHINA BAÐLANMIÞ OLDULAR;
yoltoplamuzunluk=yoltoplamuzunluk+Matris[s][uzaklik];
kruskalsehir[sayac]=Matris[s][yolindex];
sayac=sayac+1;
...
else if(bolgekodu[baslangic]!=0&&bolgekodu[bitis]!=0&&bolgekodu[baslangic]!=bolgekodu[bitis])//HER 2 ÞEHÝRDE GEZÝLMÝÞSE ancak bolge kodlarý farklýysa
{
//printf("\n\nbaslangic:%d,bitis:%d\n",baslangic,bitis);
...
//AYNI YOL DÜZERGAHINA BAÐLANMIÞ OLDULAR ;
yoltoplamuzunluk=yoltoplamuzunluk+Matris[s][uzaklik];
int x,ilk,son;
...
int main()
{
    ...
    int hata=1,i,j,x,y;//matris[satir][nereden,nereye,uzaklýk,yolindex]
...
//DUÐUM MATRÝSÝÐ DUGUME AÝT X VE Y DEÐERLERÝNÝ TUTAR.
...
CihanCihatMehmetKuruskal(dugumsayisi,Matris,Dugum,YedekMatris);//DÝKKAT BU KOD sirala(dugumsayisi,yoluzunluklari,yolindeks); ten sonra çalýþtýrýlmalý çünkü indeksler sýralandýktan sonra yol sorgulamasý yapýlacak
