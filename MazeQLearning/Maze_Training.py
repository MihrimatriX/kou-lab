...
class Labirent_egitimi(pyglet.window.Window):
    ...
    def __init__(self):
        ...
        self.obj = tkWindow()
        self.thread1 = Thread(target=self.obj.baslangic)
        ...
        pyglet.clock.schedule(self.olay_dongusu)
        pyglet.app.run()
        # control being shifted here. nice
        if self.obj.end_training_flag:
            self.bolumu_sonlandir()

    def labirent_baslat(self):
        self.set_size(self.w, self.h)
        self.set_visible(True)
        self.maze_gen = MazeGenerator(self.w, self.h)
        (self.path, self.start_point, self.goal) = self.maze_gen.labirent_olustur()

        self.start_index = self.index_bul([self.start_point[0], self.start_point[1]], self.states)
    ...
    def egitimi_baslat(self):
        ...
        goal_index = self.index_bul([self.goal_sprite.x, self.goal_sprite.y], self.states)
    ...
    @staticmethod
    def index_bul(element, array):
        for i in range(len(array)):
            if array[i][0] == element[0] and array[i][1] == element[1]:
                return i

    def muhtemel_durumlar(self, a, x, y):
        if a == 0:
            y += self.one_step
        elif a == 1:
            x += self.one_step
    ...
    def olay_dongusu(self, dt):
        if self.obj.training_flag:
            ...
            if self.restart:
                ...
                self.qetiketleri_yenile()

                self.restart = False
                return

            old_state = self.index_bul([self.agent_sprite.x, self.agent_sprite.y], self.states)

            y = random.uniform(0, 1)
            if y > self.epsilon:
                act = self.Qobj.maksimum_q_hareketi(old_state)
            else:
                act = random.randint(0, self.n_actions - 1)

            possible_state = self.muhtemel_durumlar(act, self.agent_sprite.x, self.agent_sprite.y)
            ...
            new_state = self.index_bul([self.agent_sprite.x, self.agent_sprite.y], self.states)
            ...
            self.Qobj.q_tablosunu_guncelle(old_state, act, new_state)
            ...
            if self.rewards[new_state] == self.neg_reward or self.rewards[new_state] == self.positive_reward:
                if self.rewards[new_state] == self.positive_reward:
                    print("Bu Denemede Hedefe Ulaştı!")
                self.episodes += 1
                print('Deneme', self.episodes)
        ...
        if self.obj.end_training_flag:
            pyglet.clock.unschedule(self.olay_dongusu)

        if self.obj.close_flag:
            self.close()
            pyglet.app.exit()

    def bolumu_sonlandir(self):
        episodes_array = []
        for i in range(self.episodes-1):
            episodes_array.append(i+1)
        plt.plot(episodes_array, self.actions_performed)
        plt.xlabel('Bölümler')
        plt.ylabel('Bölüm Başına Hareket Sayısı')
        plt.show()
